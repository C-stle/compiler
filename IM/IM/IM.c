...
char* newlabel();
char* newtemp();
char* strcat_my(char*, char*, char*, char*, char*, char*, char*);
char* strcat_my2(char*, char*, char*, char*, char*, char*, char*, char*, char*, char*);
void code_gen(Ty_Node_Ptr);
...
tokentype lexan() {
	...
	while (1) {
		...
		case 5: // 글자 + 가 나온 후의 처리를 담당하는 상태.
			...
			break;
		case 6:
			token.kind = 51; strcpy(token.sub_kind, "AND");		 strcpy(token.str, "&&");
			return token;
...
char* strcat_my2(char* desti, char *s1, char* s2, char* s3, char* s4, char* s5, char* s6,
				char* s7, char* s8, char* s9) {
	int total_leng = 0;
	char* result;
	total_leng = strlen(desti) + strlen(s1) + strlen(s2) + strlen(s3) + strlen(s4)
		+ strlen(s5) + strlen(s6) + strlen(s7) + strlen(s8) + strlen(s9) + 10;
	result = (char*)malloc(total_leng);
	strcpy(result, desti);
	strcat(result, s1);
	strcat(result, s2);
...
void code_gen(Ty_Node_Ptr cur) {
	Ty_Node_Ptr L_, L, L1, S, S1, S2, id_t, id1_t, id2_t, rop,
		BE, BE1, BT, BT1, BF, BF1, E, E1, T, T1, F, num_t;

	if (cur->nodesym.kind == 0)
		return;
	switch (cur->rule_no_used) {
		...
		case 1: // L -> S
			...
			break;
		case 2:
			L = cur; L1 = cur->children[0]; S = cur->children[2];
		...
		case 4:
			...
			break;
		case 5:
			S = cur; BE = cur->children[2]; S1 = cur->children[5]; S2 = cur->children[7];
		...
		case 21:
			...
			break;
		case 22:
			BF = cur; id1_t = cur->children[0]; rop = cur->children[1]; id2_t = cur->children[2];
			BF->code = strcat_my2("if ", id1_t->nodesym.str, " ", rop->nodesym.str, " ",
				id2_t->nodesym.str, " goto ", BF->tru, "\ngoto ", BF->fal);
			strcat(BF->code, "\n");
			break;
		case 23:
...
int main() {
	...
	if (!fps) {
		printf("File open of source file failed.\n");
		getchar();
	}
	Root = Parsing(fps);
	fclose(fps);
